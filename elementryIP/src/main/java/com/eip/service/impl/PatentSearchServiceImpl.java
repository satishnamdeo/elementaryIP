...
//			SOLRUtil.indexFile("c:/Solr/patents/SegmentedClaims.xml");
	...
	public JSONObject viewPatent(String searchQueryStr,String projectId) {
		JSONObject patent=new JSONObject();
		...
		SolrInputDocument sd=SOLRUtil.getPatentById(searchQueryStr,projectId);
	...
	public String addSegment(PatentDo patentDo) { 
		...
		try{
			if(patentDo.getClaimIndependentNo()!=null){
				...
				SolrInputDocument solrInputDoc=SOLRUtil.getPatentById(patentDo.getPatentId(),"0");
