...
<%@ taglib prefix="sec" uri="http://www.springframework.org/security/tags"%>
	...
	<div class="container">
		...
		<div class="row">
			...
			<form action="<%=request.getContextPath() %>/createproject.html">
				<label>Project Name :</label>
				<input type="text" name="ProjectName" id="ProjectName" />
				<label>Add Collaborator :</label>
				<select name="users" multiple="multiple" size="3">
				<c:forEach var="userList" items="${userList}" varStatus="count">
			<option value="${userList.userId}">${userList.firstName} ${userList.lastName }</option>
			</c:forEach>
		...
		<div class="row" style="margin-top: 1%">
			...
			<c:if test="${projectList.size() == 0}"><label style="color: blue"> User has not  created any project.</label></c:if>
			</div>
		</div> 
